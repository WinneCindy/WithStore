...
- (void)viewDidLoad {
    ...
    [super viewDidLoad];
    SaoEr = [BaseDomain getInstance:NO];
    ...
    imageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 64, 0.8 * self.view.frame.size.width, 0.8 * self.view.frame.size.width)];
    ...
    upOrdown = NO;
    num =0;
    _line = [[UIImageView alloc] initWithFrame:CGRectMake(50, 155, 220, 2)];
...
-(void)animation1
{
    if (upOrdown == NO) {
        num ++;
        _line.frame = CGRectMake(CGRectGetMinX(_line.frame), 155+2*num, CGRectGetWidth(_line.frame), CGRectGetHeight(_line.frame));
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    if ([metadataObjects count] >0)
    {
        ...
        if (stringValue.length > 0) {
            ...
            [params setObject:type[1] forKey:@"ticket_id"];
            
            [SaoEr postData:URL_scan_ticket PostParams:params finish:^(BaseDomain *domain, Boolean success) {
                if ([self checkHttpResponseResultStatus:domain]) {
                    [self alertViewShowOfTime:@"兑换成功" time:0.5];
                    [self.navigationController popViewControllerAnimated:YES];
